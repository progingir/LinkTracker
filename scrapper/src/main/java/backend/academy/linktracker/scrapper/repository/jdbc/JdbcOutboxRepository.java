...
public class JdbcOutboxRepository implements OutboxRepository {
    ...
    public OutboxMessageEntity save(OutboxMessageEntity entity) {
        jdbcClient.sql("INSERT INTO outbox_messages (id, payload, created_at, status, attempts) VALUES (?, ?, ?, ?, ?)")
    ...
    public List<OutboxMessageEntity> findMessagesToProcess(int limit) {
        return jdbcClient.sql("SELECT id, payload, created_at, status, attempts FROM outbox_messages WHERE status = 'PENDING' OR status = 'FAILED' ORDER BY created_at ASC LIMIT ?")
            .param(1, limit)
